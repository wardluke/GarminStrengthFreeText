...
class JsonDataResourcesView extends WatchUi.View {
    ...
    public function onUpdate(dc as Dc) as Void {
        ...
        // Clear the view
        var textWidth = dc.getWidth() / 2;
        var textHeight = dc.getFontHeight(Graphics.FONT_XTINY) + 5;
        dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_BLACK);
        dc.fillRectangle(0, 0, dc.getWidth(), dc.getHeight());
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_BLACK);
        var headerText = "Press Menu";
        dc.drawText(textWidth, 5, Graphics.FONT_XTINY, headerText, Graphics.TEXT_JUSTIFY_CENTER);

        if (null != _curLoadedJsonResource) {
            // Draw the current loaded JSON record as text on the screen
            dc.drawText(textWidth, textHeight, Graphics.FONT_XTINY, getDisplayString(_curLoadedJsonResource), Graphics.TEXT_JUSTIFY_CENTER);
        }
    }
	...
	var displayString = "Dictionary\nJSON Resource\n";
        ...
        var exercises = dictionary["exercises"] as Array<Dictionary>;
        

        if(exercises instanceof Array){
            for(var index=0;index<=dictionary.keys().size();index++){
                System.println(exercises[index]);
                System.println(exercises[index]["exercise"]);
                System.println(exercises[index]["reps"]);
                System.println(exercises[index]["time"]);
            }

        }
        System.println("key- reps - " + dictionary["reps"]);
        return displayString;
    }
